...
2. **샌드박스 격리 (Sandbox Isolation)**: 위험도가 높은 작업(Zone 2: 외부 코드 실행, 패키지 설치 등)은 반드시 `jarvis-box` 샌드박스에서 수행합니다.
   - **관리**: `core_harness/openclaw-sandbox.yaml`을 통해 선언적으로 관리합니다.
   - **라우팅**: `nemoclaw onboard --non-interactive`를 사용하여 환경을 최신 상태로 유지하거나, `openshell sandbox exec -n jarvis-box -- [command]` 형식을 사용합니다.
...
1. **사전 검증 (Verification First)**: 작업을 시작하기 전, 필요한 도구나 의존성이 실제로 존재하는지 확인합니다. (예: 샌드박스 내 `openclaw` 설치 여부 확인)
...
3. **사용자 개입 필수 시점**: 에이전트는 다음 상황에서 반드시 작업을 멈추고 사용자에게 지시를 요청해야 합니다.
   ...
   - **파괴적 작업**: 컨테이너 삭제, 자격 증명 재설정 등 되돌리기 어려운 작업 수행 시.
